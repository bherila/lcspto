...
using MvcMembership;
using ElixCms.Areas.Admin.Models.UserAdministration;

namespace ElixCms.Areas.UserAdministration.Controllers.UserAdministration
{
	...
	public class UserAdministrationController : Controller
	{
        ...
        public UserAdministrationController()
            : this(
                ...
                new SmtpClientProxy(new SmtpClient() { Timeout = 2000 })) {
        }
		...
		[AcceptVerbs(HttpVerbs.Post)]
		public RedirectToRouteResult Details(object id,
		...
		[AcceptVerbs(HttpVerbs.Post)]
		public RedirectToRouteResult ResetPassword(object id, string answer)
		{
			var user = _userService.Get(id);
			var newPassword = _passwordService.ResetPassword(user, answer);

			var body = ResetPasswordBody + newPassword;
            user.Comment += "Password reset to " + newPassword;
            _userService.Update(user);
        ...
        public ActionResult CreateUser(string username, string password)
        {
            ...
            Membership.CreateUser(username, password, null, null, null, true, null, out createStatus);

            return RedirectToAction("Index", new { status = createStatus.ToString() });
        }
	}
}